...
| 25 | 2026-08-25 | Workflow startup_failure invisible | quality-gate.yml dead 24 days; checks API showed only other workflows green | YAML job at column 0 = schema error; startup failures create NO check-runs | Re-indent job under jobs:; instrument steps with ::error diagnostics | Monitor workflow-runs endpoint (not just check-runs) after any workflow file change; actionlint now active via pre-commit |
| 26 | 2026-08-25 | Two permission suites asserted opposite policies | Port brought suite asserting ask while root suite asserted deny for same commands; stale one never ran in ci.yml | Ported tests not reconciled against live SSoT before merge | Aligned stale assertions to canonical policy (ADR-045/046); 157/157 unified | When porting test suites, diff their EXPECTATIONS against current SSoT first — contradictory suites are silent time-bombs |
| 27 | 2026-08-25 | Write tool overwrote existing file without checking | Bulk rewrite clobbered a 122-line test suite that arrived with concurrent main commits (132 deletions in stat exposed it) | No existence check before Write on repo with parallel sessions | Restored from git show + fused both suites | ALWAYS glob/git-check file existence before Write in repos with active parallel work |
| 28 | 2026-08-26 | Duplicate #requires resurfaces mid-file | Adding line-1 #requires -Version 5.1 while -Version 7 sat at line 26 → parse error "parameter specified more than once" | Directive hidden after comment block evades head-3-lines gates | Single directive moved to line 1 | When fixing #requires, grep the WHOLE file for all directives, not just the top |
