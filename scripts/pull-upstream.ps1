...
if($LASTEXITCODE-ne0){Write-Warning "Fetch failed";Pop-Location;exit 1}
...
if([string]::IsNullOrEmpty($up)){$uf=@(git ls-tree -r --name-only $rem|?{$_-notmatch'^skills/|^scripts/|^\.'-and$_-notlike'*/'});$lf=@(git ls-tree -r --name-only $loc|?{$_-notmatch'^\.agents/skills/|^scripts/|^\.'-and$_-notlike'*/'})}
else{$uf=@(git ls-tree -r --name-only $rem -- "$up"|%{if(-not[string]::IsNullOrEmpty($lo)-and$lo-ne$up){$_.Replace($up,$lo)}else{$_}});$lf=@(git ls-tree -r --name-only $loc -- "$lo"|%{$_})}
$us=@($uf|Sort-Object -Unique);$ls=@($lf|Sort-Object -Unique)
$n=@($us|?{$_-notin$ls});$c=@($us|?{$_-in$ls});$o=@($ls|?{$_-notin$us})
if($l-eq'Root files'){$n=$n|?{$_-notmatch'\.md$|LICENSE'}}
$mod=@()
if($l-ne'Root files'){$uh=@{};if([string]::IsNullOrEmpty($up)){git ls-tree $rem|%{$p=$_-split'\s+';$uh[$p[3]]=$p[2]}}else{git ls-tree -r $rem -- "$up"|%{$p=$_-split'\s+';$fp=$p[3];if(-not[string]::IsNullOrEmpty($lo)-and$lo-ne$up){$fp=$fp.Replace($up,$lo)};$uh[$fp]=$p[2]}};$lh=@{};if([string]::IsNullOrEmpty($lo)){git ls-tree $loc|%{$p=$_-split'\s+';$lh[$p[3]]=$p[2]}}else{git ls-tree -r $loc -- "$lo"|%{$p=$_-split'\s+';$lh[$p[3]]=$p[2]}};$c|%{if($uh[$_]-and$lh[$_]-and$uh[$_]-ne$lh[$_]){$mod+=$_}}}
Write-Host "--- $l ---"
if($n.Count){Write-Host "NEW $($n.Count)";$n|%{Write-Host "  + $_"}}
if($mod.Count){Write-Host "MOD $($mod.Count)";$mod|%{Write-Host "  ~ $_"}}
if($o.Count){Write-Host "OURS $($o.Count)";$o|%{Write-Host "  - $_"}}
if(-not$n.Count-and-not$mod.Count-and-not$o.Count){Write-Host "(none)"}
$nu+=$n;$mo+=$mod;$ou+=$o}
Write-Host "New:$($nu.Count) Mod:$($mo.Count) Ours:$($ou.Count)"
if($Mode-eq'Apply-New'){$sf=$nu|?{$_-match'^\.agents/skills/' -or $_-match'^scripts/'};$sk=@($nu|?{$_-notin$sf})
...
if($LASTEXITCODE-eq0){if($up-ne$f){if(Test-Path $up){Move-Item -Path $up -Destination $f -Force}};$null=git add $f 2>&1;$app++}else{Write-Warning "FAILED $up";$fail++}}
...
if($LASTEXITCODE-eq0){if($up-ne$TargetFile){$pd=Split-Path $TargetFile -Parent;if(-not[string]::IsNullOrEmpty($pd)-and-not(Test-Path $pd)){New-Item -ItemType Directory -Path $pd -Force|Out-Null};Move-Item -Path $up -Destination $TargetFile -Force};Write-Host "Done. git diff --cached $TargetFile"}else{Write-Warning "Failed $up"}}
Pop-Location;exit 0
