...
.SYNOPSIS
    ...
    - Uses FileSystemWatcher + Wait-Event (NOT polling)
    - Cleans up signal file + temp callback
...
Describe "BabyAGI async push notification" {
    ...
    It "Invoke-TaskAsync has 300s timeout on Wait-Event" {
        ...
        ($src -match 'Wait-Event.*\$maxWait') | Should -BeTrue
    }
    ...
    It "Invoke-TaskAsync uses Join-Path for absolute result file path" {
