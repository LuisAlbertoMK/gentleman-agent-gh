...
Write-Step "Global config" {
    if ($Force -or -not (Test-Path $globalCfg)) {
        ...
        $cfg=@{'$schema'="https://opencode.ai/config.json";mcp=@{context7=@{enabled=$true;type="remote";url="https://mcp.context7.com/mcp"};engram=@{command=@("engram","mcp","--tools=mem_save,mem_search,mem_context,mem_session_summary,mem_get_observation,mem_save_prompt,mem_current_project,mem_judge");type="local"}};permission=@{bash=@{"*"="allow";"git commit *"="ask";"git push *"="ask";"git push --force *"="ask";"git push --delete *"="ask";"git rebase *"="ask";"git reset *"="ask";"git merge *"="ask";"git branch -D *"="ask";"git stash drop *"="ask";"gh pr merge *"="ask"};read=@{"*"="allow";"**/.env"="deny";"**/.env.*"="deny";"**/credentials.json"="deny";"**/secrets/**"="deny";"*.env"="deny";"*.env.*"="deny"}};agent=$existingAgents}
