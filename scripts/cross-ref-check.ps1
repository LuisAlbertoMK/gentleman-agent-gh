...
if(-not $Quiet){Write-Host "[2/8] SKILL.md..."-N};$ms=@()
...
if(-not $Quiet){Write-Host "[3/8] INDEX count..."-N};$ac=($sd | Where-Object {$_.Name -ne '_shared'}).Count
...
if(-not $Quiet){Write-Host "[4/8] junctions..."-N};$mg=@()
...
if(-not $Quiet){Write-Host "[6/8] cross-refs..."-N};$br=@()
...
if(-not $Quiet){Write-Host "[7/8] config_refs..."-N};$mc=@();$crp='config_refs:\s*(.+)'
