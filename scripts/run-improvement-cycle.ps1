...
$agsz=0
if(Test-Path $agp){$agsz=(gi $agp).Length}
...
if(Test-Path $cdir){
  dir $cdir -Directory|%{
    ...
    if(Test-Path $mp){$null=$sf.Add((gi $mp))}
  }
}
$sc=$sf.Count
$tsz=0.0
...
$big5=$sf|sort Length -Descending|select -First 5
Write-Host "Largest:" -ForegroundColor Gray
$big5|%{$r=$_.FullName.Replace($RepoRoot,"").TrimStart("\");$k=[math]::Round($_.Length/1024.0,1);Write-Host "  $r : $k KB" -ForegroundColor DarkYellow}
$vb=New-Object System.Collections.ArrayList
$tvb=0.0
$sf|%{if($_.Length-gt3072){$null=$vb.Add($_);$tvb+=$_.Length}}
$vc=$vb.Count
$vkb=[math]::Round($tvb/1024.0,1)
if($vc-gt0){Write-Host "Verbose (>3KB): $vc files, $vkb KB" -ForegroundColor Magenta;if($AutoCompress){Write-Host "  Auto-compress on" -ForegroundColor Green}}else{Write-Host "No verbose skills" -ForegroundColor Green}
Write-Host "Cross-ref..." -ForegroundColor Cyan
$cre=0;$crw=0;$crc=$true
$crs=Join-Path $PSScriptRoot "cross-ref-check.ps1"
if(Test-Path $crs){$cro=& $crs -Json 2>$null;$crj=$cro-join"`n";try{$crp=$crj|ConvertFrom-Json;$cre=$crp.errors.Count;$crw=$crp.warnings.Count;if($cre-gt0){Write-Host "Xref: $cre ERROR(S)" -ForegroundColor Red;$crp.errors|%{Write-Host "  $_" -ForegroundColor Red};$crc=$false}else{Write-Host "Xref: OK ($crw warn)" -ForegroundColor Green}}catch{Write-Host "Xref: parse error" -ForegroundColor Red;$cre=1}}else{Write-Host "Xref: script not found" -ForegroundColor Yellow}
...
if(Test-Path $lp){ac $lp -Value $le -Encoding UTF8;Write-Host "Logged as $id" -ForegroundColor Green}else{Write-Host "WARNING: .learnings not found" -ForegroundColor Yellow}
