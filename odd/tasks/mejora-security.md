...
| 2026-09-18 | 5 (dispatch por tipo de proyecto) | plan-execution + adversarial-breaker | freeze `HEAD-4e8acda8-e69de29b`; commit `feat(skills): add project-type dispatch` (SHA en `git log`); 2 archivos (project-mapper: sección `Security Playbook Dispatch` — API→auth-hardening, SPA→best-practices+auth-hardening, backend→data-quality, containers→container-security, IA→llm-security + regla monorepo/mixed dispatch por package; SKILLS-INDEX.md: fila Security del Quick Groups extendida con todos los carriers + pointer a project-mapper) — **desvío del plan**: no se tocó security-scanner (sin headroom); dispatch en project-mapper (router natural, headroom real 292 B) en pipe-format compacto en vez de tabla md (mismo patrón que slices 1-4), cross-refs en SKILLS-INDEX (sin gate de budget) cubren el set completo de carriers; frontmatter intacto, sin bumps; YAML OK; check-token-budget passed; regression 101/101; breaker: 3/3 casos (repo API+SPA, monorepo mixto, proyecto IA) con file:line — APPROVED |
| 2026-09-18 | 6 (close breaker CRITICAL+HIGH, 9/25 gaps) | plan-execution + adversarial-breaker | freeze `HEAD-ebd9c2a-e69de29b`; commit `fix(skills): close breaker CRITICAL+HIGH gaps, document follow-ups` (SHA en `git log`, sin SHAs propios); 6 skills extendidos: best-practices (Service Worker: same-origin+scope+worker-src+auditoría SW; DOM clobbering form/iframe), data-quality (SSTI sandbox, XXE entidades externas/DTD, deserialización pickle/yaml.load), container-security (checklist caps extendido + cap-add FORBIDDEN individuales: NET_ADMIN, SYS_MODULE, SYS_RAWIO, DAC_OVERRIDE, SYS_PTRACE, SYS_CHROOT, SYS_BOOT, IPC_LOCK), infra-audit (subPath `..` traversal, sin bump), auth-hardening (JWT jku/x5u allowlist, x5c trust store, kid server-side), project-mapper (híbridos Next/Nuxt/Remix → ambos playbooks) + `## Known Limitations (follow-ups)` MEDIUM/LOW en este doc; **desvío gate-forced** (precedente e47d202f, bumps MÍNIMOS ceil(size/1.1)): best-practices 3450→3992, data-quality 3340→3895, container-security 2500→2807, auth-hardening 3200→3471, project-mapper 2916→3157 (infra-audit 2500 sin bump, 2746≤2750); frontmatter intacto salvo bumps; YAML OK; check-token-budget passed; regression 101/101; breaker: re-ataque 9/9 vectores con file:line — APPROVED |

## Known Limitations (follow-ups)

Vectores MEDIUM/LOW del dictamen NO-GO del breaker (25 gaps: 9 CRITICAL+HIGH cerrados en slice 6). Los siguientes NO se implementan en este slice — documentados como follow-ups (honestidad anti-falsa-confianza):

HPP | GraphQL batching | CSP bypass (JSONP/AngularJS) | postMessage opener | cookie prefixes (`__Secure-`/`__Host-`) | WebSocket CSRF | NoSQLi | path traversal | prototype pollution | ReDoS | hostPath `/proc` | runc (container escape) | admission bypass | RBAC impersonate | ML monorepo | detection bypass | custom monorepo tools | GraphQL aliasing
