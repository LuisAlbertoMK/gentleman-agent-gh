...
6. **Owner-paste pendiente `prompts/shared/_permission-templates.md` (drift docs↔runtime, follow-up R10-S4):** el bloque "Orchestrator (gentle-MK)" todavía lista `python *`, `python3 *`, `node *` = deny (`prompts/shared/_permission-templates.md:53-55`), mientras el runtime (`scripts/lib/opencode-base.json`, 77 reglas) y el SSoT del gate (`scripts/opencode-config/shared-deny-rules.json`, `node *`/`python *` = allow) ya tienen `python *`/`node *` = allow (dirección ADR-046/#693). Es el ÚNICO drift restante y es DOCUMENTAL (ningún test/script lee ese .md — verificado). No fue aplicable en R10-S4 porque el runtime deniega `edit prompts/**` a los agentes → requiere paste de owner. Cambio exacto: eliminar esas 3 líneas del bloque Orchestrator; opcional alinear con R10-S4 agregando `ftp *`/`scp *`/`rsync *` = deny + `git clone *` = ask a los bloques Orchestrator y Write-Capable. `confidence: high` en drift / `medium` en que ningún flujo depende del .md.
7. **Paridad de gate en `scripts/opencode-config/shared-deny-rules.json` (R11, no urgente):** ese archivo (83 reglas, SSoT del gate `permission-gate-lib.ps1`) NO tiene `ftp *`/`scp *`/`rsync *`/`git clone *` aunque el runtime SÍ los tiene tras R10-S4. Mecanismos distintos (gate vs opencode bash-permission), por eso no es urgente, pero conviene paridad. Al agregarlos, el test `tests/permission-rules-consistency.Tests.ps1:40-46` solo exige curl/wget/ssh, así que no requiere cambio forzoso. `confidence: medium`.
