...
- gentle-MK, gentleman-deep, gentleman-quick, gentleman-codex use `{file:prompts/shared/_core-behavior-gp.md}` (GP-specific: tool constraints, return format, no unachievable orchestration rules)
...
- gentle-MK inherits global permissions (orchestrator role)
