...
- `EventEmitter.on()` in `process.on('message')` or `EventSource` for streaming LLM responses — each `new EventSource(url)` creates connections
...
**Detection:** Heap snapshot → look for `EventListener` objects under `(event listeners)` or `(compiled code)`. Use `bun --heap-prof-md` or `node --inspect` Memory panel.

**Fix:**
```typescript
// ❌ Leak
process.on('message', handler);
// handler never removed — accumulates per session

// ✅ Fix: use Effect.Listener.add or explicit cleanup
...
`setInterval` / `setTimeout` without clear → prevents GC of entire closure scope.
...
// ✅ Fix: Wrap in Effect Schedule with bounded count
...
```typescript
// ❌ Leak: each handler keeps whole array alive
...
1. Bounded size — `Map` + max capacity + eviction
2. TTL-based — auto-expire entries after time
3. WeakRef-based — let GC decide
...
**Detection:** Heap snapshot → filter by string size. Look for strings > 1KB with unexpected retention paths.
...
**Specific pattern:** `zipLatest` + `async` Stream + non-emitting Stream → infinite fiber creation ([Effect Issue #3200](https://github.com/Effect-TS/effect/issues/3200)).

```typescript
// ❌ Leak: zipLatest with async Stream that never emits
Stream.zipLatest(
  Stream.async<never>(emit => { /* never emits */ }),
  otherStream
)

// ✅ Fix: Always ensure async streams emit or use bounded PubSub
...
|------|----------|-----|------|---------|
| **clinic doctor** | Triage (±15s run) | ❌ | ✅ | `npx clinic doctor -- node app.js` |
| **clinic heapprofiler** | Allocation flamegraph | ❌ | ✅ | `npx clinic heapprofiler -- node app.js` |
| **clinic flame** | CPU hot paths | ❌ | ✅ | `npx clinic flame -- node app.js` |
| **clinic bubbleprof** | Async bottlenecks | ❌ | ✅ | `npx clinic bubbleprof -- node app.js` |
| **`bun --heap-prof-md`** | Heap profile on exit | ✅ | ❌ | `bun --heap-prof-md script.ts` |
| **`bun --cpu-prof-md`** | CPU profile in markdown | ✅ | ❌ | `bun --cpu-prof-md script.ts` |
| **`bun --inspect`** | Chrome DevTools (partial) | ✅⚠️ | ✅ | `bun --inspect app.ts` |
| **Node `--inspect`** | Full Chrome DevTools | ❌ | ✅ | `node --inspect app.js` |
| **`Bun.gc(true)`** | Force sync GC + mimalloc cleanup | ✅ | ❌ | In-code |
| **`process.memoryUsage()`** | Raw RSS/heap numbers | ✅ | ✅ | In-code |
| **`0x`** | V8 flame graphs | ❌ | ✅ | `npx 0x -- node app.js` |

### 2.2 Heap Snapshot Analysis

**Node:** Use `node --inspect`, open `chrome://inspect`, go to Memory tab → "Take heap snapshot". Filter by constructor name, look for:
- `(string)` — large string accumulation
- `(closure)` — leaked closures
- `(array)` / `(object properties)` — Map entries
- `Detached DOM tree` — if browser context

**Bun:** `bun --heap-prof-md` generates a markdown file on exit with `# Heap Profile` sections listing object counts per type ([Bun docs](https://bun.com/docs/project/benchmarking)). This is the closest Bun has to heap snapshots — no interactive DevTools yet, but the markdown output is grep-friendly.

### 2.3 Allocation Tracking

**Node:** Memory panel → "Allocation sampling" records allocations per function. Also `--trace-gc` logs GC events showing frequency and duration.
...
- Fibers are ~40 bytes each — 10K fibers ≈ 400KB ([Effect DeepWiki](https://deepwiki.com/Effect-TS/effect/2.2-fiber-based-concurrency))
- An Effect is a lazy description (zero cost until run) — no GC pressure from creating effects
...
```typescript
// Safe pattern — finalizers run in reverse on scope close
Effect.scoped(
  Effect.gen(function* () {
...
**Leak-prone patterns:**
- ❌ `Effect.scoped` wrapping only the **acquire** but not the **use** phase
- ❌ `Scope.make()` + `Scope.close()` manually without error handling in between
- ❌ Forgetting `.pipe(Effect.scoped)` on scoped effects

### 3.3 Pool Management

```typescript
// Safe — TTL prevents unbounded connection growth
Pool.makeWithTTL({
...
**Leak risk:** `Pool.make` without `timeToLive` → connections accumulate if `max` is high and connections are slow to return.

### 3.4 PubSub / Queue Bounding

```typescript
// ❌ Unbounded — message backlog grows with slow consumers
PubSub.unbounded<string>()

// ✅ Bounded — backpressure or dropping
...
```typescript
// Supervisor tracks all forked fibers — prevents fire-and-forget accumulation
...
setInterval(async () => {
  ...
  if (fibers > 1000) {
    console.error(`WARNING: ${fibers} active fibers — possible leak`)
    Bun.gc(true)
  }
}, 60000)
```
...
1. **Conversation history** (prompts + responses) — 4-50KB per exchange
2. **Tool results** — could be megabytes for file reads or code generation
3. **Context summary** — compressed but still grows
4. **Token usage metadata** — array of `{ input, output, timestamp }` records
...
```typescript
// Track by token count, not message count — safer for variable-length messages
...
|----------|-----------|----------------|-------------|----------|
| **Bounded LRU** (Map + capacity) | Low | ✅ Exact | ❌ Objects stay alive | Fixed-size cache with known access pattern |
| **TTL-based** (Map + expiry) | Low | ⚠️ Approximate | ❌ Objects stay alive until expiry | Short-lived data (tool results, API responses) |
| **LFU** (frequency tracking) | Medium | ✅ Exact | ❌ More metadata overhead | Hot data has high reuse (skill lookups) |
| **WeakRef-based** | Medium | ❌ GC decides | ✅ Auto-evicts | Cache where data can be re-fetched |
| **Size-based** (bytes, not count) | High | ✅ Exact | ❌ Must measure | Variable-size cache (LLM responses vary 10x) |
| **Combined** (TTL + capacity) | Medium | ✅ Best | ❌ | Recommended default |
...
Memory leaks in agent systems surface slowly — after 2+ hours of use. A 5-minute test suite won't catch them. CI detection must specifically exercise leak-prone paths under controlled baselines.
...
```bash
# Each test file runs in fresh process — prevents cross-test memory bleed
...
- **No ballooning**: If RSS reaches limit, you're dead — no second chance
...
// Bun (gentleman-vMK)
--smol  // JIT: 256→64MB, GC heap: 32→4MB, RSS: 47→13MB (-72%)
```
...
- Reduce max context messages from 20 → 5
- Skip token tracking metadata accumulation
- Drop tool result caching entirely (use WeakRef)
- Disable skill auto-load — load only explicitly needed skills
...
setInterval(async () => {
  ...
  if (rss > MEMORY_WARN) {
    ...
    console.error(`MEMORY WARNING: RSS ${(rss/1024/1024).toFixed(0)}MB — snapshot saved`)
  }
}, 30000)
```
...
| Metric | Warning | Critical | Action |
|--------|---------|----------|--------|
| RSS > 200MB | ✅ | — | Degrade cache |
| RSS > 80% of OOM limit | — | ✅ | Emergency summarization |
| GC recovery < 5% over 3 checks | ✅ | — | Notify + mark for profiling |
| Heap growth > 1MB/min | — | ✅ | Restart process |
| Fiber count > 5000 (Effect) | ✅ | ✅ | Supervisor audit + log |
...
|----------|-------------|-----------|
| **Fiber lifecycle** | Fork → interrupt → GC | < 500 bytes/cycle |
| **Conversation history** | Add → summarize → GC | Flat after summary |
| **Cache eviction** | Fill → evict → GC | Flat after eviction |
| **Timer cleanup** | Set → clear → GC | No handle leak |
| **Event listeners** | Add → remove → GC | No listener leak |
| **Pool connections** | Acquire → release → GC | Flat after TTL |
| **Large response** | Process → nullify → GC | < 10KB retained |
...
it('should handle rapid allocation without OOM', async () => {
  for (let i = 0; i < 500; i++) {
    ...
    // Should not grow indefinitely — GC should reclaim between iterations
  }
  Bun.gc(true)
...
it('should produce no new detachments after operation', () => {
  ...
  // Compare — if DetachedDOMTree count increases, leak
...
- [ ] **Set `--smol` for gentleman-vMK** — cuts baseline RSS ~72%
...
- [ ] **Wrap resources in `Effect.scoped`** with `acquireRelease` — audit existing uses

### Medium Priority (next iteration)

- [ ] **Replace `PubSub.unbounded` with `PubSub.bounded`** everywhere
- [ ] **Check all `Effect.fork` calls** — prefer `Effect.fork` (supervised) over `forkDaemon`
...
- [ ] **Profile with clinic heapprofiler** for Node (opencode-vMK) — 2x/year or after major changes
- [ ] **Profile with `bun --heap-prof-md`** for Bun (gentleman-vMK) — 2x/year
...
| # | Source | URL |
|---|--------|-----|
| 1 | Clinic.js — Official docs | https://www.clinicjs.org/ |
| 2 | Clinic.js — Heap Profiler | https://www.clinicjs.org/documentation/heapprofiler/ |
| 3 | Effect.ts — Scope docs | https://effect.website/docs/resource-management/scope/ |
| 4 | Effect.ts — Supervisor docs | https://effect.website/docs/observability/supervisor/ |
| 5 | Effect Issue #3200 — zipLatest leak | https://github.com/Effect-TS/effect/issues/3200 |
| 6 | EffectPatterns — Community patterns (300+) | https://github.com/PaulJPhilp/EffectPatterns |
| 7 | Effect DeepWiki — Fiber concurrency | https://deepwiki.com/Effect-TS/effect/2.2-fiber-based-concurrency |
| 8 | Effect Pool.ts — makeWithTTL | https://effect-ts.github.io/effect/effect/Pool.ts.html |
| 9 | Google DevTools — Fix memory problems | https://developer.chrome.com/docs/devtools/memory-problems/ |
| 10 | MDN — WeakRef | https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/WeakRef |
| 11 | JS.info — WeakRef & FinalizationRegistry | https://javascript.info/weakref-finalizationregistry |
| 12 | Node.js — Understanding Memory Tuning | https://nodejs.org/en/learn/diagnostics/memory/understanding-and-tuning-memory |
| 13 | V8 — Trash Talk: Orinoco GC | https://v8.dev/blog/trash-talk |
| 14 | V8 — Orinoco Parallel Scavenger | https://v8.dev/blog/orinoco-parallel-scavenger |
| 15 | Bun docs — Benchmarking / profiling | https://bun.com/docs/project/benchmarking |
| 16 | Bun API — `Bun.gc()` | https://bun.com/reference/bun/gc |
| 17 | Lucio Durán — Bun v2 Runtime Internals | https://lucioduran.com/blog/bun-v2-runtime-internals-deep-dive |
| 18 | The Register — Bun 1.1.13 memory fixes | https://www.theregister.com/software/2026/04/21/bun-1113-out-with-memory-fixes-as-dev-complain-of-leaks/5221154 |
| 19 | DEV.to (Axiom) — Node.js profiling | https://dev.to/axiom_agent/nodejs-performance-profiling-in-production-v8-flame-graphs-clinicjs-and-heap-snapshots-2d70 |
| 20 | HireNodeJS — Clinic.js profiling 2026 | https://www.hirenodejs.com/blog/nodejs-clinic-js-profiling-2026 |
| 21 | Red Hat — Node.js memory in containers | https://developers.redhat.com/articles/2025/10/10/nodejs-20-memory-management-containers |
| 22 | Zoer.ai — Bun vs Node mem comparison | https://zoer.ai/posts/zoer/bun-vs-nodejs-memory-usage-comparison |
| 23 | froxell.com — Bun vs Node 2026 | https://www.froxell.com/blog/bun-runtime-breakdown-nodejs-2026 |
| 24 | OneUptime — Optimize Bun performance | https://oneuptime.com/blog/post/2026-01-31-bun-performance-optimization/view |
| 25 | dev.to (Shafayet) — TS performance | https://dev.to/shafayeat/performance-optimization-with-typescript-dcj |
| 26 | Node.js — GC Traces | https://nodejs.org/en/learn/diagnostics/memory/using-gc-traces |
| 27 | Node.js — Using Heap Profiler | https://nodejs.org/en/learn/diagnostics/memory/using-heap-profiler |
| 28 | Node.js — Using Heap Snapshot | https://nodejs.org/en/learn/diagnostics/memory/using-heap-snapshot |
| 29 | Effect v4 Beta — Bundle size blog | https://effect.website/blog/effect-v4-beta |
| 30 | ACM Queue — Idle-time GC | https://queue.acm.org/detail.cfm?id=2977741 |
| 31 | IJIRT — V8 Memory for Node.js | https://ijirt.org/publishedpaper/IJIRT182978_PAPER.pdf |
| 32 | PlainEnglish.io — WeakRef smarter memory | https://javascript.plainenglish.io/weakrefs-finalizationregistry-smarter-memory-management-in-javascript-366a3acf08f6 |

---

> **Next step**: Implement the checklist items in priority order. Start with runtime flags (`--smol` / `--max-old-space-size`) and bounded caches — these are the highest-impact, lowest-effort items.
