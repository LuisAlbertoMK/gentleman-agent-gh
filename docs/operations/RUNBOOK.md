...
| **Subagent "completed" but no output** | (a) Free-tier model hit output truncation, (b) stdout truncated by verbose verification, (c) model fell back to `general` due to `mode: primary` not being delegable | Run `git diff --name-only HEAD` — if empty, treat as silent failure. Retry with narrower scope (max 2 files). If still empty → escalate | Post-delegation git verification is MANDATORY. Never trust "completed" without diff proof. See `gentle-MK.md` post-delegation gate |
...
**MANDATORY for ALL subagent delegations** — enforced by `gentle-MK.md`:
...
| Auto agents deny `python` even after `sync-all` | G1 fix: `scripts/lib/opencode-base.json` (`permission.bash` python/python3) now allows python (ADR-046); `shared-deny-rules.json` and `permission-templates.json` `auto` already allow it. The GLOBAL `~/.config/opencode/opencode.jsonc` is profile-level and NOT regenerated by `sync-all` — its auto agents still deny python | Apply the python → allow fix to every auto agent's `permission.bash` in `~/.config/opencode/opencode.jsonc`, then **restart OpenCode** | `opencode.jsonc` is profile-level; audit it whenever ADR-046 changes the SSoT |
...
```powershell
# Local (default — 100% offline, no external APIs):
.\scripts\ui-specialist-pairing.ps1 -Target src/components -Mode full -Vision

# Cloud (requires VISION_ANALYZE_OLLAMA_CLOUD=1 and sanitized screenshots — no PII):
$env:VISION_ANALYZE_OLLAMA_CLOUD="1"
$env:OLLAMA_CLOUD="1"
