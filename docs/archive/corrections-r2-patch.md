...
### C1. Qwen3.5-35B â€” model does not exist
...
- **Corrected text**:
  ...
  > **âš ï¸ Data source unclear**: Values below appear extrapolated or from an unverifiable source.
  ...
  *Data pending re-benchmark â€” no Qwen3.5-35B exists in the Qwen3 model family.*
  ```
- **Priority**: **CRITICAL** â€” Factual error about a non-existent model. All inference data in that table is from an unverifiable source.

---

### C2. VRAM formula â€” flat 1.2x multiplier incorrect

- **File**: `docs/research/optimization/ram-cpu-gpu-optimization.md`
- **Location**: Line 149
- **Current**:
  ```markdown
  **Formula**: `VRAM = Params Ã— (Bits/8) Ã— 1.2` (20% KV cache overhead)
  ```
- **Corrected text**:
  ```markdown
  > **Replaced by context-dependent formula below**:

  **Weight memory**: `VRAM_weights = Params Ã— (Bits/8)`

  **KV cache memory**: `VRAM_kv = 2 Ã— num_layers Ã— num_kv_heads Ã— head_dim Ã— seq_len Ã— bytes_per_value`
  ...
  **Rule of thumb**: At 4K context, ignore KV cache (~1%). At 128K+, KV cache dominates.
  ...
  `ponytail: simplified formula â€” exact depends on architecture (GQA ratio, layers, head_dim). For design docs, use the context-dependent table above.`
  ```
- **Priority**: **CRITICAL** â€” Flat 20% is misleading by up to 12x depending on context length.

---

### C3. KV Cache table for 70B ignores GQA â€” ~8x overstatement
...
- **Corrected text**:
  ...
  **KV Cache Memory per Token (with GQA)**:
  ...
  not 64. This reduces KV cache by ~8Ã— vs the naive calculation.
  ...
  **Without GQA** (older architectures like Llama 1/2):
  ...
  > is ~8Ã— overstated for modern 70B models. Always check the KV head count.
  ```
- **Priority**: **CRITICAL** â€” ~8Ã— factual overstatement on KV cache for modern 70B models.

---

## High Priority Corrections

### H1. LangGraph "9% overhead vs raw" â€” uncited, actual 15-38%
...
- **Corrected text**:
  ...
  - **Token cost**: 15-38% overhead vs raw (varies by workflow complexity; [source: LangGraph
    benchmarks 2025])
  ```
- **Priority**: **HIGH** â€” 9% is outside the documented range; uncited claim.

---

### H2. CrewAI "15-31%" â€” actual 18-48%+
...
- **Corrected text**:
  ...
  ```
- **Priority**: **HIGH** â€” Understated lower bound by 3pp, upper bound by 17pp.

---

### H3. AutoGen "31% overhead" â€” actual 8-28% latency, 20-50% cost
...
- **Corrected text**:
  ...
  ```
- **Priority**: **HIGH** â€” Single number conflates latency and cost; both ranges differ.

---

### H4. Speculative decoding â€” 2-4x only at batch size 1
...
- **Corrected text**:
  ...
  > **âš ï¸ Production caveat**: These numbers are at **batch size 1**. In production with
  > high concurrency (batch size 8+), draft model acceptance rate drops and speedup
  > narrows to **1.1-1.2Ã—**. Draft decoding is most effective for latency-sensitive
  > single-user or agent use cases.
  ```
- **Priority**: **HIGH** â€” Without this caveat, the 2-4Ã— figure is misleading for production.
...
- **Corrected text** â€” add after line 79:
  ```markdown
  > **âš ï¸ Tradeoff**: ACON incurs **+15-30s latency on A100** per compression pass
  > (mentioned in approaches-comparison.md Â§3.4). This is acceptable for long-running
  ...
  ```
- **Priority**: **HIGH** â€” Omission of a significant latency tradeoff creates misleading picture.

---

## Medium Priority Corrections

### M1. Get-Content "23x faster" â€” actual 6-14x
...
- **Corrected text**:
  ...
  ```
- **Priority**: **MEDIUM** â€” Overstated by ~2x but directionally correct.
...
- **Current**:
  ```markdown
  **Real-world case**: $42K â†’ $2.1K monthly (95% savings) via LLMLingua compression alone.
  ```
- **Corrected text**:
  ```markdown
  **Real-world case** (single study, synthesis-heavy workload, high-traffic):
  $42K â†’ $2.1K monthly (95% savings) via LLMLingua compression.

  > **âš ï¸ Context**: This is a specific case study. Actual savings depend on traffic volume,
  ...
  ```
- **Priority**: **MEDIUM** â€” Factually correct but presented as general expectation.

---

### M3. LightAgent "no deps" â€” has 13+ deps
...
- **Corrected text**:
  ...
  ```
- **Priority**: **MEDIUM** â€” "No deps" is factually wrong; intent ("no heavy framework lock-in")
  is correct.
...
- **Corrected text**: Move to a new section:
  ...
  (renumber existing sections 5â†’7, 6â†’8, 7â†’9, 8â†’10)
...
- **Priority**: **MEDIUM** â€” Not factually wrong, but placement under "Memory" conflates
...
| Qwen3.5-35B latency | token-context.md:157 | **CRITICAL** â€” no such model |

- **Priority**: **MEDIUM** â€” No incorrect data, but missing transparency about evidence quality.
...
- **Priority**: **MEDIUM** â€” Research credibility issue, not data error.
...
- **Corrected text**: Add a footnote:
  ...
  ```
- **Priority**: **LOW** â€” Subjective ratings are clear-enough in context.

---

### L2. Token-context doc â€” ACON description mentions Qwen3-14B âœ… (already correct)
...
|---|----------|---------|----------|
| 1 | `optimization/profiling-baseline.md` | Measure current perf before any optimization â€” RAM, CPU, GPU baselines for both projects | High |
...
| 6 | `architecture/cross-project-sync-strategy.md` | gentleman-agent-gh â†” opencode-vmk sync: junction management, conflict resolution | Medium |
...
|---------|------|-------------|----------|
| Â§1.5 PowerShell RAM | ram-cpu-gpu.md:57-70 | Startup latency benchmarks, module loading costs, JIT warmup time | High |
| Â§3 GPU/vRAM | ram-cpu-gpu.md:137-178 | GQA KV cache correction (see C3), context-dependent formula (see C2) | Critical |
| Â§2.3 ACON | token-context.md:67-79 | Latency tradeoff (15-30s), batch compression option | High |
| Â§2.4 Context Caching | token-context.md:81-88 | Semantic caching latency vs savings tradeoff (TTL, invalidation) | Medium |
| Â§5 Hardware Impact | token-context.md:148-165 | GQA-aware KV cache table, remove/fix Qwen3.5-35B table | Critical |
