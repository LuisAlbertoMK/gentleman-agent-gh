...
## Rules
1. Compress at ORANGE (60%), never RED (80%). 2. Always start L1 first (60-70%) before escalating. 3. First drift → force YELLOW + L1 immediately. 4. 3+ edits same file → STOP, summarize, commit, re-read. 5. Every 25 calls → `mem_save(topic_key=checkpoint/session-state)` (`mem_save(checkpoint/session-state)`). 6. Same point 2× or hallucination → force RED, break session.
...
## Drift + Force-RED
65% failures = drift: re-reads same content, re-states question, references unsaid (re-reads, re-states, unsaid refs) → force YELLOW + L1. Force-RED: same point 2× · self-contradiction · "as I mentioned" w/o source (referencing nothing).
## Anti-Patterns
...
Externalized to keep skill ≤3KB (ADR-048). Consult for DAG wiring detail (DAG wiring, escalation):
