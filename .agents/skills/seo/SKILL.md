...
## Technical SEO
**Crawlability**: valid robots.txt · XML sitemap in Search Console · canonical URLs · `noindex` only on staging/duplicates · HTTPS + HSTS · short hyphen-separated URLs
**Sitemap**: `<lastmod>` over `<changefreq>`/`<priority>` (ignored by Google) · 50K URLs / 50MB max per file · sitemap index for >1
...
## Audit checklist
**Critical**: indexable · unique title 50-60ch · meta desc 150-160ch · H1 with keyword · canonical URL · HTTPS
