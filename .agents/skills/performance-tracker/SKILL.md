...
---
## Scope
Score app PERFORMANCE (not agent — use `auto-metrics`). Covers: Mobile (Android/iOS), Desktop (Win/Mac/Linux), Web.
## 6 Dimensions (1-10)
### Load
9-10: M <1.5s / D <1s / W LCP<1.5 TTI<2 | 7-8: <2.5s/<2s/<2.5/<3.5 | 5-6: <4s/<3.5/<4/<5 | 3-4: <6s/<5/<6/<7 | 1-2: >6s/>5/>6/>7
### Render
...
**Trend** (every 10 scores OR session end): `mem_search(query="perf-score:", limit=20)` → compare prev(5) vs recent(5). Dim drop >0.5 → `gap-analysis`.
**Template**: `## Trend: {date} | {N} scores | Dim|Prev|Recent|Delta | ... | **Avg**|**X.X**|**X.X**|**±X.X** | Verdict: improving/stable/declining`
## Action by Avg
