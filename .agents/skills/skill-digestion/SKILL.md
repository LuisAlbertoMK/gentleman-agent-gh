...
## DIGEST ON LOADExtract only what's needed. Skip background/examples when context is tight.| Context | Load strategy ||---------|--------------|| <60% | Full skill || 60-80% (YELLOW) | Rules + decision tree only || >80% (RED) | 1-line summary + critical rules |**How**: match skill→context → load Critical Patterns + Decision Tree + Commands.
