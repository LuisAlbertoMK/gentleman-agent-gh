...
---
## When to Use
Persistent memory - save, search, dreaming, session lifecycle via Engram MCP.
...
Batch: normal→every3t, low→session-end via `topic_key="batch/{session-topic}"`, high→immediate. Validate: `scripts/engram-validate.ps1`→run after `mem_save`.
## Dreaming
...
## Temporal Edges (P1-3)
Zep-style: `Get-TemporalChain` sorts `createdAt`→`deltaHours`; re-rank `mem_search` by recency. Detail→`docs/skills/engram-protocol/reference.md` § Extended (ADR-048).
## Session Close
`!close`→`mem_session_summary`(Goal/Discoveries/Accomplished/Next/Files). Flush low batch. Gate: `close-session.ps1` verifies summary. `!score`/`!dream`. Mandatory unless pure chat.
