...
## Visual verification with vision-analyze
After fixing UI issues:
1. Capture screenshot of fixed component
2. Run `analyze-screenshot.ps1 -Mode design`
3. Verify fixes match design intent
4. Save before/after to `docs/metricas/ui-improvements/`

## Integration with vision-analyze
Cross-reference code findings with visual output:
- Layout code issues → verify visually with vision-analyze
- Typography rules → check rendering in browser screenshot
- Spacing values → confirm visual consistency
- Animation rules → verify compositor-only animations
