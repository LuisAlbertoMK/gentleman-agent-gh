...
- **Decision**: Adopt the v2 protocol — metrics M1-M9 (Gate 16/16, E2E 702/0, skill sizes avg ≤2,000B & 0 >3KB, Score ≥9.5, PSSA <50 warnings & 0 gate regressions, opencode.json ≤65,536B, cross-ref 0 errors, BenchmarkSeconds stable, .project.json freshness ≤1 day); budget max 3 cycles per session block and ≤45 min per cycle; diminishing-return threshold: marginal improvement <10% vs prior cycle → STOP; human checkpoint per cycle; ICE prioritization I×C/E; 3 subagents per cycle.
