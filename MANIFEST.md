...
| Área | Qué hago | Dónde |
|------|----------|-------|
| **Skills** | Crear, mantener, mejorar los 68 skills | `.agents/skills/` |
...
| **Bridge** | Reportar hallazgos que afecten a opencode-vMK o global | `D:\TEMP\opencode-bridge.jsonl` |
...
|--------|-------|----------------------|
| **Core engine de opencode-vMK** (`packages/core/`, `packages/opencode/`) | Es código del fork, no mío | Reporto error en el bridge con evidencia |
...
| **Binario opencode-vMK.exe** | Es compilado del fork | Verifico que existe, no lo rebuildéo |
...
> Cosas que detecto en mi dominio y que pueden mejorar a opencode-vMK o global. Las dejo en el bridge como propuesta, no como fix.

| Detección | Sugerencia | Para quién |
|-----------|------------|------------|
| Skill nuevo requiere feature del runtime | "Este skill necesita soporte para X en el core" | opencode-vMK |
| Config drift detectado | "Tus agents difieren de mi canonical — sync sugerido" | opencode-vMK, global |
| Performance issue en skills | "Este skill hace N llamadas, podría optimizarse en runtime" | opencode-vMK |
| YAML frontmatter roto en SKILL.md | "Archivo X tiene YAML inválido — revisar" | opencode-vMK (por si es su skill loader) |
| Mejora en protocolo de bridge | "Propongo agregar campo X al formato JSONL" | Ambos |
...
> Si sos **opencode-vMK** (el fork) o **opencode global** (oficial), copiá este template y completalo en `D:\opencode\MANIFEST.md` o `~/.config/opencode/MANIFEST.md`.
...
Su contraparte para opencode-vMK debería vivir en `D:\opencode\MANIFEST.md`.
Ambos referenciados desde el bridge principal.
