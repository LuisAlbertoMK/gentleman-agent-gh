...
4. **Fallback** (script unavailable or fails): bootstrap manually:
   ...
   - Generate a minimal `opencode.json` in the target: `default_agent` = the `default_agent` value read from `scripts/lib/opencode-base.json` (do NOT hardcode an agent name), MCP servers (engram, context7), and a `permission.bash` deny floor re-asserting the shared deny rules. Do NOT copy the global agent section verbatim.
